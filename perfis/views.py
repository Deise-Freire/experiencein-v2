...
def convidar(request, *args, **kwargs):
    try:
        ...
        raise exceptions.NotFound(
            'Não foi encontrado um perfil com o id imformado')
    perfil_logado = get_perfil_logado(request)
    if perfil_a_convidar != perfil_logado:
        ...
        return response.Response(
            {
                'mensagem':
                f'Convite enviado com sucesso para {perfil_a_convidar.email}.'
            },
            status=status.HTTP_201_CREATED)
    raise exceptions.ParseError(
        'Você não pode convidar o perfil com o id informado.')
...
def aceitar(request, *args, **kwargs):
    perfil_logado = get_perfil_logado(request)
    try:
        convite = Convite.objects.filter(convidado=perfil_logado).get(
            id=kwargs['convite_id'])
    except:
        raise exceptions.NotFound(
...
#@login_required - retirado no projeto final
